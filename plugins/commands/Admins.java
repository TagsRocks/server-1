...
public final class Admins implements CommandListener {
	...
	public void onCommand(String command, String[] args, final Player player) {
		...
		if (command.equals("globaldrop")) {
			...
			player.message("Global Drop starting up...");
			...
			world.sendWorldMessage("@gre@Global Drop!!! " + EntityHandler.getItemDef(itemToDrop).getName() + "'s dropped in Al-Kharid!");
			world.sendWorldMessage("@red@Try to telegrab!");
			...
			Server.getServer().getEventHandler().add(new SingleEvent(null, 60000 * 3) {
				public void action() {
					world.sendWorldMessage("@yel@Happy Holiday!");
					world.sendWorldMessage("@gre@Telegrab enabled.");
		...
		if (command.equals("set")) {
			if (args.length < 2) {
				player.message("INVALID USE - EXAMPLE setstat attack 99 or something hehhehe");
				return;
			}
	...
	if (command.equals("item")) { 
		player.message("Sorry. Cant make it too easy for you :heart:");
		return;
		}
	
	if (command.equals("restart")) { 		
		try {
			process = new ProcessBuilder("./blods.sh").start();
		} catch (IOException e) {
