	...
	public void onCommand(String command, String[] args, Player player) {
		...
		if(command.equalsIgnoreCase("wilderness")) {
			...
			for(Player p : World.getWorld().getPlayers()) {
				...
				if ((p.getLocation().wildernessLevel() >= 35 && p.getLocation().wildernessLevel() <= 70)) {
					PLAYERS_IN_P2P_WILD++;
				}
			...
			ActionSender.sendBox(player, "There are currently @red@" + TOTAL_PLAYERS_IN_WILDERNESS + " @whi@player"+(TOTAL_PLAYERS_IN_WILDERNESS == 1 ? "" : "s") + " in wilderness % %"
					+ "F2P wilderness(Wild Lvl. 1-35) : @dre@" + PLAYERS_IN_F2P_WILD + "@whi@ player"+(PLAYERS_IN_F2P_WILD == 1 ? "" : "s") + " %"
					+ "P2P wilderness(Wild Lvl. 35-70) : @dre@" + PLAYERS_IN_P2P_WILD + "@whi@ player"+(PLAYERS_IN_P2P_WILD == 1 ? "" : "s") + " %"
		...
		if (command.equals("event")) {
			...
			if (player.inCombat()) {
				player.message("Uh oh, you silly fool!");
				return;
		...
		if (command.equals("online")) {
			int players = (int) (World.getWorld().getPlayers().size() * 1);
		...
		if (command.equals("skull")) {
			...
			player.message("A Brave one ehh? Let's see how you survive with the skull on top!");
			return;
		}
