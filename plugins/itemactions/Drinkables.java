...
public class Drinkables implements InvActionListener, InvActionExecutiveListener {
	...
	public void onInvAction(Item item, Player player) {
		...
		case 318:
				...
				player.message("You drink the " + item.getDef().getName() + ".");
				player.getInventory().remove(item);
				player.getInventory().add(new Item(181));
				sleep(1500);
						player.message("It tastes awesome and you hunger more!!");
						for (int stat = 0; stat < 3; stat++) {
						player.getSkills().setLevel(stat, player.getSkills().getLevel(stat) - 20);
					}
					if (player.getSkills().getLevel(16) <= player.getSkills().getMaxStat(16)) {
						player.getSkills().setLevel(16, player.getSkills().getLevel(16) + 20);
						sleep(500);
						player.message("You feel invincible!");
					}
					break;		
